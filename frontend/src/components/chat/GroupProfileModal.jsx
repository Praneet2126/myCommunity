...
function GroupProfileModal({ isOpen, onClose, chat, cityName, onMembersChanged }) {
  ...
  const fetchItinerary = async () => {
    try {
      ...
      const data = await response.json();
      console.log('[GroupProfileModal] Itinerary fetch response:', data);
      ...
      if (data.success && data.data) {
        ...
        // Debug: Log day numbers being received
        if (data.data.days) {
          console.log('[GroupProfileModal] Day numbers received:', data.data.days.map(d => ({day: d.day, type: typeof d.day, activities: d.activities?.length || 0})));
        }
        
        // Debug: Log hotels if present
        if (data.data.hotels) {
          console.log('[GroupProfileModal] Hotels received:', data.data.hotels.length, 'hotels');
        }
        
        setGeneratedItinerary(data.data);
      } else {
        // No itinerary found, clear state
        console.log('[GroupProfileModal] No itinerary found in response');
        setGeneratedItinerary(null);
      }
    } catch (error) {
      console.error('[GroupProfileModal] Error fetching itinerary:', error);
    }
  };
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-4">
          ...
          {activeTab === 'itineraries' && (
            <div className="space-y-3">
              {/* Generated Itinerary - Show if available (already fetched for this chat) */}
                ...
                <div className="bg-gradient-to-br from-orange-50 to-amber-50 border-2 border-orange-200 rounded-lg p-4">
                  <div className="flex items-center justify-between mb-3">
                    <div>
                      ...
                      <p className="text-xs text-gray-600 mt-1">
                        ...
                        {generatedItinerary.hotels && generatedItinerary.hotels.length > 0 && ` · ${generatedItinerary.hotels.length} hotel${generatedItinerary.hotels.length > 1 ? 's' : ''}`}
                      </p>
                    </div>
                  ...
                  {generatedItinerary.hotels && generatedItinerary.hotels.length > 0 && (
                    <div className="mb-4 bg-white rounded-lg p-3 border border-orange-100">
                      ...
                      <div className="space-y-2">
                        {generatedItinerary.hotels.map((hotel, hotelIndex) => (
            ...
            <div className="space-y-2">
              ...
              {/* Empty State */}
              {(!generatedItinerary || !generatedItinerary.days || generatedItinerary.days.length === 0) && savedItineraries.length === 0 && (
