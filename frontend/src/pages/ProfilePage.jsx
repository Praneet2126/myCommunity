...
function ProfilePage() {
  ...
  const fetchSavedItineraries = useCallback(async () => {
    if (!token) {
      console.log('No token available for fetching itineraries');
      return;
    }
    ...
    try {
      const response = await getUserItineraries(token);
      console.log('Fetched itineraries response:', response);
      if (response.success) {
        console.log('Number of saved itineraries:', response.data?.length || 0);
        setSavedItineraries(response.data || []);
      } else {
        console.error('Failed to fetch itineraries:', response);
      }
    } catch (error) {
  ...
  // Fetch itineraries when tab changes to 'itineraries'
  useEffect(() => {
  ...
  // Transform saved itinerary to display format
  const transformItineraryForDisplay = (savedItinerary) => {
    const activities = [];
    savedItinerary.days.forEach(day => {
      day.activities.forEach(activity => {
        activities.push({
          day: day.day,
          time: `${activity.start_time}${activity.end_time ? ` - ${activity.end_time}` : ''}`,
    ...
    // Handle city_id - it can be a string or an object
    let cityId = savedItinerary.city_id;
    if (typeof cityId === 'object' && cityId !== null) {
      cityId = cityId._id || cityId;
    }
    ...
    return {
      ...
      activities,
      estimatedCost: savedItinerary.estimated_cost,
    ...
    <div className="min-h-screen bg-gray-50">
            ...
            {loadingItineraries ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {savedItineraries.map((savedItinerary) => {
                  const displayItinerary = transformItineraryForDisplay(savedItinerary);
                  return (
                    <div key={savedItinerary._id} className="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100 hover:shadow-md transition-shadow">
                      <div className="p-6">
                        <div className="flex items-start justify-between mb-4">
                          <div className="flex-1">
                            <h3 className="font-bold text-lg text-gray-900 mb-2">{displayItinerary.title}</h3>
                            <div className="flex items-center gap-3 text-sm text-gray-600">
                              <span>{displayItinerary.days} days</span>
                              {displayItinerary.estimatedCost && (
                                <>
                                  <span>•</span>
                                  <span>{displayItinerary.estimatedCost}</span>
                                </>
                              )}
                            </div>
                          </div>
                        ...
                        {displayItinerary.tags.length > 0 && (
              ...
              <div className="text-center py-20">
                ...
                <p className="text-gray-500">Save itineraries from the popular itineraries section to view them here</p>
              </div>
            )}
          </div>
        )}
      ...
      {viewingItinerary && (
        <ItineraryDetailModal
          ...
          cityId={viewingItinerary.city_id}
        />
      )}
